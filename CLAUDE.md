...
| Реестр сетей: экспорт отчёта PDF/PPTX по витрине | `handlers/reports.go` (задания, как `sales_export_jobs.go`); `services/report_snapshot.go` (каталог блоков, проверка запроса, снимок = ответ витрины); `reports/` (страничная модель `pages.go`, векторные графики через общий холст `canvas.go`, fpdf, ручной OOXML, токены `theme.go`, встроенные шрифт и шаблон); `repository/report_jobs_repo.go`; `models/report.go` | `components/ReportExportDialog.tsx`, `api/reports.ts`, `types/reports.ts` |
