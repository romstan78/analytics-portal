...
(роуты,     (HTTP, вход-  (формулы,      (SQL)
 ...
 rate limit)     ↘ reports/ — рендер PDF/PPTX из снимка; зависит только от models/
...
| Реестр сетей: экспорт отчёта PDF/PPTX по витрине | `handlers/reports.go` (задания, как `sales_export_jobs.go`; токен печати); `services/report_snapshot.go` (каталог блоков, проверка запроса, снимок = ответ витрины); `reports/` (страничная модель `pages.go` + `spec.go`, печатная модель `print.go`, PPTX из снимков и нативных таблиц `pptx.go`, `chrome/` — chromedp к headless Chromium); `repository/report_jobs_repo.go`; `models/report.go` | `pages/PrintReport.tsx` (страница печати для Chromium), `components/report/*`, `print.css`, `components/ReportExportDialog.tsx`, `api/reports.ts`, `types/reports.ts` |
...
| Реестр сетей → «Итоги» → «Экспорт отчёта» | `components/ReportExportDialog.tsx` | `handlers/reports.go`, `reports/` |
| **Реестр сетей** → карточка сети: «Профиль сети», «Цены и SKU», «План и факт», «Инвестиции OPEX», «Прогноз», «Комментарии», «История» | `pages/NetworkRegistry.tsx`, `components/NetworkDetailView.tsx`, `NetworkPricesTab.tsx`, `NetworkPlanGrid.tsx`, `NetworkOpexTab.tsx`, `NetworkForecastTab.tsx` | `handlers/network.go`, `network_opex.go`, `services/network_service.go`, `network_opex_service.go`, `network_forecast_service.go` |
...
4. **Миграции goose встроены в бинарник и применяются при старте.** Файлы
   `backend/migrations/0NN_*.sql` пронумерованы до 035; изменение уже применённого файла
