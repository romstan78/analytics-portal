...
### Факт по планам реестра сетей

Планы, прогнозы и процент инвестиций КАМ вводит в портале, а факт приходит выгрузкой отгрузок — в интерфейсе он доступен только для чтения:
...
| `Год`, `Квартал` | да | квартал 1–4 |
| `Факт, руб` | одна из двух | фактический объём |
| `Факт инвестиций, руб` | одна из двух | фактические инвестиции |

Файл проверяется целиком до записи: пустые ключи, нечисловые и отрицательные значения, неверный квартал и дубли строк перечисляются одним списком. Сети, которых нет в реестре, останавливают загрузку и выводятся с подсказкой похожих названий:
