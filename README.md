...
Повторная загрузка требует отдельной защитной команды и очищает только
промо-данные demo-БД:

```bash
make demo-db-reset
```

Для входа задайте `DEMO_BOOTSTRAP_PASSWORD` в локальном `.env` и выполните:
