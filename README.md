...
`sync_data.py` переносит интернет-продажи из OLAP и обновляет нормализованную таблицу. Старый импорт промо пока намеренно заблокирован: до его перевода на атомарную staging/MERGE-схему запускать его нельзя.
