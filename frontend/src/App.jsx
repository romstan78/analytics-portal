...
// Создаем современную тему (SaaS / Apple-like)
const modernTheme = createTheme({
  palette: {
    mode: 'light',
    primary: { 
      main: '#6366f1', // Трендовый индиго
      light: '#818cf8',
      dark: '#4f46e5',
      contrastText: '#ffffff',
    },
    ...
    MuiButton: {
      styleOverrides: {
        root: {
          ...
          '&:hover': {
            boxShadow: 'none',
            transform: 'translateY(-1px)', // Легкое приподнятие при наведении
          }
        },
        contained: {
    ...
    MuiDataGrid: {
      styleOverrides: {
        root: {
          ...
          '& .MuiDataGrid-columnSeparator': { display: 'none' }, // Убираем вертикальные линии в шапке
        },
      },
    },
    MuiDialog: {
  ...
  return (
    <Box sx={{ p: 4 }}>
      ...
      <Box sx={{ 
        ...
        <Typography variant="h3" gutterBottom sx={{ fontWeight: 700, color: 'text.secondary' }}>
          {title}
        </Typography>
    ...
    <ThemeProvider theme={modernTheme}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: 'background.default' }}>
        <Routes>
          ...
          <Route path="/promo-analysis" element={<PromoAnalysis />} />
          <Route path="/sales-analysis" element={
            <PlaceholderPage title="Анализ продаж" description="Динамика продаж по периодам" />
          } />
          <Route path="/network-registry" element={
            <PlaceholderPage title="Реестр сетей" description="Справочник торговых сетей" />
          } />
          <Route path="/turnover" element={
            <PlaceholderPage title="Оборачиваемость" description="Анализ оборотов запасов" />
          } />
          <Route path="/like-for-like" element={
            <PlaceholderPage title="Продажи Like For Like" description="Сравнение продаж LFL" />
          } />
        </Routes>
      </Box>
    </ThemeProvider>
