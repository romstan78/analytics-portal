...
export default function BudgetTable({ data, expanded, onExpand, sales, composition, source, kind, delta, busy, onEdit, onPromo }: Props) {
  ...
  <TableBody>{data.brands.filter(b => b.brand !== 'Нераспределённый остаток пула').map(b => <Fragment key={b.brand}>{lines(b)}{expanded.includes(b.brand) && b.networks.map(n => <Fragment key={n.networkId}>{lines(n, true)}</Fragment>)}</Fragment>)}{lines(data.total, false, true)}</TableBody>
